...
●インストール
 zip書庫ファイルの場合、「Tascher162.zip」を適当なフォルダに解凍して下さい。
 インストーラの場合、「Tascher162_Setup.exe」を実行してください。
 ...
 *のある項目の動作は設定画面から変更することができます。
 ...
 [設定画面項目一覧]
  [全般]
   ...
   │ │ │   ・文字が入力されてから確定するまでの時間を[高度な設定]-[Migemo検索を開始するまでの遅延時間]で指定することができます。
   │ │ ├[使用しない]
   ...
   │ │   ・確定するまでの時間を[高度な設定]-[「候補のウインドウが1つなら確定」遅延時間]で指定することができます。
   │ │
   │ └[連文節検索を行う(Migemo)]
  ...
  [表示方法]
   ├[マウスの移動]
   │ ├[左上][上][右上][左][右][左下][下][右下]
   │ │・マウスカーソルをそれぞれ画面隅に移動させるとウインドウリストを表示します。
   │ │・自動的にマウスカーソルがウインドウリスト内に移動します。
   │ │
   │ └[マウスホイール回転時のみ]
   │   ・マウスカーソルをそれぞれ画面隅に移動させ、かつマウスホイールを回転させるとウインドウリストを表示します。
   │
   └[ホットキー]
  ...
  [高度な設定]
   │   ・Ver.1.61までの隠し設定を設定ダイアログから編集できるようにしました。
   ├[動作]
   │ ├[ドラッグ中はマウスホバーで選択]
   ...
   │
   ├[インクリメンタルサーチ]
   │ ├[「候補のウインドウが1つなら確定」遅延時間]
   │ │ ・「候補のウインドウが1つなら確定」で切り替えるまでの時間を指定します。
   │ │ ・デフォルトの値は300です。
   │ │ 
   │ └[Migemo検索を開始するまでの遅延時間]
...
●Q&A風備忘録
 ...
 A3.Tascherを一度終了させ、「Tascher.cfg」の[ShortcutKey]セクション全て([ShortcutKey]も含め)を削除し、Tascherを再度起動するとショートカットキー設定が初期化されます。
 Q4.「MMHook.dll」とは何ぞや。
 A4.カーソル移動やマウスアウトを検出するためのライブラリです。
    (特にキーボード派の方で)不要であれば、「MMHook_.dll」のようにリネームしてください。
 Q5.マウス四隅移動をしてもウインドウリストが表示されない。一部ウインドウの最大化が出来ない。ウインドウの情報が取得出来ない。
 A5.通常権限でTascherを起動すると、管理者権限で動作しているプロセスに対して一部動作を行うことが出来ません。
    Tascherを管理者権限で起動させて下さい。
