...
●インストール
 「Tascher163.zip」を適当なフォルダに解凍して下さい。
...
●使用方法
 ...
 [設定画面項目一覧]
  [全般]
   ...
   │ │   ・最前面かつ最小化されているウインドウ、デスクトップ、キャンセルはカウントしません。
  ...
  [表示方法]
   ...
   │ │
   │ ├[マウスホイール回転時のみ]
   │ │・マウスカーソルをそれぞれ画面端に移動させ、かつマウスホイールを回転させるとウインドウリストを表示します。
   │ │
   │ ├[判定までの時間]
  ...
  [コマンド]
   ...
   ├[引数]
   │  │・実行するファイルに渡す引数を指定します。
   │  └[フルパス %F][ディレクトリパス %D][ファイル名 %N][タイトル %T][ハンドル(10進数) %H][ハンドル(16進数) %X][クラス名 %C][% %%]
   │    ・対応する文字列をそれぞれ置換します。
...
●隠し設定
 ...
 ・元の設定に戻す場合、併記しているデフォルト値を入力するかキーごと削除してください。
  ...
  [ShowWindow]
   ...
   │    ────────┘
   │
   ├MouseClipCursorMode=0
   │ ・[表示方法]-[マウスの移動]で[カーソルを○○へ移動]で移動してから一定時間マウスカーソルの移動範囲をウインドウリスト内に制限します。
   │ ・[表示方法]-[共通]-[「選択項目にカーソルを移動」が有効なら表示時に移動]が有効な場合、
   │   移動範囲はウインドウリスト内ではなく、選択項目の範囲になります。
   │ ・0…無効、1…上辺及び下辺のみ制限、2…矩形内に制限
   │ ・選択項目の変更やメニューを表示すると解除されます。
   ├MouseClipCursorTime=0
   │ ・MouseClipCursorModeで移動範囲を制限する時間を指定します。
   ├HotKeyClipCursorMode=0
   │ ・[表示方法]-[ホットキー]で[カーソルを○○へ移動]で移動してから一定時間マウスカーソルの移動範囲をウインドウリスト内に制限します。
   │ ・[表示方法]-[共通]-[「選択項目にカーソルを移動」が有効なら表示時に移動]が有効な場合、
   │   移動範囲はウインドウリスト内ではなく、選択項目の範囲になります。
   │ ・0…無効、1…上辺及び下辺のみ制限、2…矩形内に制限
   │ ・選択項目の変更やメニューを表示すると解除されます。
   ├HotKeyClipCursorTime=0
   │ ・HotKeyClipCursorModeで移動範囲を制限する時間を指定します。
   │ 
   ├LeftCornerDistance=3
   ├RightCornerDistance=3
   ├TopCornerDistance=3
   └BottomCornerDistance=3
      ・[表示方法]-[マウスの移動]で画面端と判断する端からの距離を指定します。


●Q&A風備忘録
 Q1.ショートカットキー設定を初期化したい。
 A1.Tascherを一度終了させ、「Tascher.cfg」の[ShortcutKey]セクション全て([ShortcutKey]も含め)を削除し、Tascherを再度起動するとショートカットキー設定が初期化されます。
 Q2.マウス四隅移動をしてもウインドウリストが表示されない。
    一部ウインドウの最大化が出来ない。
   「movetomonitor」ができない。
    一部情報が取得出来ない(プロセス名が取得できない、管理者権限で動作しているのにElevated=Noとなる)。
    等々。
 A2.通常権限でTascherを起動すると、管理者権限で動作しているプロセスに対して一部動作を行うことが出来ません。
